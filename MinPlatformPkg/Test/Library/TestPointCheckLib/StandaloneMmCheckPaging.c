  ...
  while (BaseAddress < BaseAddressEnd) {
    ...
    if (IsCode) {
      if ((PageEntry == NULL) || ((*PageEntry & IA32_PG_P) == 0)) {
        ...
        //
        // Check if it is StandaloneMM SaveState
        //
        if (!IsStandaloneMmSaveState (BaseAddress)) {
  ...
  )
{
  ...
  for (Index = 0; Index < MemoryAttributesTable->NumberOfEntries; Index++) {
    DEBUG ((DEBUG_INFO, "StandaloneMmMemoryAttribute Checking 0x%lx - 0x%x\n", Entry->PhysicalStart, EFI_PAGES_TO_SIZE ((UINTN)Entry->NumberOfPages)));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "==== TestPointCheckStandaloneMmPaging - Exit\n"));
  return EFI_SUCCESS;
}
