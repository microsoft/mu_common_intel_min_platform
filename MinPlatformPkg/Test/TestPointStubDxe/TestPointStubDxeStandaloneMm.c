  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    DEBUG ((DEBUG_INFO, "SmiHandlerTestPoint: Locate MmCommunication protocol - %r\n", Status));
    return ;
  }
  ...
  DEBUG ((DEBUG_INFO, "StandaloneMmTestPointDatabaseSize - 0x%x\n", mSmmTestPointDatabaseSize));
...
EFI_STATUS
EFIAPI
TestPointStubDxeStandaloneMmEntryPoint (
