  ...
  )
{
  ...
  if (PciDeviceInfo != NULL) {
    VTDLIB_DEBUG (
      (DEBUG_INFO, "PCI Device Information (Number 0x%x, IncludeAll - %d):\n",
    ...
    for (Index = 0; Index < PciDeviceInfo->PciDeviceDataNumber; Index++) {
      VTDLIB_DEBUG (
        (DEBUG_INFO, "  S%04x B%02x D%02x F%02x\n",
  ...
  )
{
  ...
  for (Index = 0; Index < VTD_ROOT_ENTRY_NUMBER; Index++) {
    if ((RootEntry[Index].Uint128.Uint64Lo != 0) || (RootEntry[Index].Uint128.Uint64Hi != 0)) {
      VTDLIB_DEBUG (
        (DEBUG_INFO, "  RootEntry(0x%02x) B%02x - 0x%016lx %016lx\n",
    ...
    for (Index2 = 0; Index2 < VTD_CONTEXT_ENTRY_NUMBER; Index2++) {
      if ((ContextEntry[Index2].Uint128.Uint64Lo != 0) || (ContextEntry[Index2].Uint128.Uint64Hi != 0)) {
        VTDLIB_DEBUG (
          (DEBUG_INFO, "    ContextEntry(0x%02x) D%02xF%02x - 0x%016lx %016lx\n",
  ...
  )
{
  ...
  for (Index = 0; Index < VTD_ROOT_ENTRY_NUMBER; Index++) {
    if ((ExtRootEntry[Index].Uint128.Uint64Lo != 0) || (ExtRootEntry[Index].Uint128.Uint64Hi != 0)) {
      VTDLIB_DEBUG (
        (DEBUG_INFO, "  ExtRootEntry(0x%02x) B%02x - 0x%016lx %016lx\n",
    ...
    for (Index2 = 0; Index2 < VTD_CONTEXT_ENTRY_NUMBER/2; Index2++) {
      ...
      {
        VTDLIB_DEBUG (
          (DEBUG_INFO, "    ExtContextEntryLower(0x%02x) D%02xF%02x - 0x%016lx %016lx %016lx %016lx\n",
    ...
    for (Index2 = 0; Index2 < VTD_CONTEXT_ENTRY_NUMBER/2; Index2++) {
      ...
      {
        VTDLIB_DEBUG (
          (DEBUG_INFO, "    ExtContextEntryUpper(0x%02x) D%02xF%02x - 0x%016lx %016lx %016lx %016lx\n",
  ...
  )
{
  ...
  VTDLIB_DEBUG (
    (DEBUG_INFO, "DXE [%ld]: SetAttribute SourceId = 0x%04x, Address = 0x%lx, Length = 0x%lx, IoMmuAccess = 0x%lx, %r\n",
  ...
  )
{
  ...
  if ((IqaReg.Uint64 & BIT11) == 0) {
    ...
    DEBUG ((
      DEBUG_VERBOSE,
      "[0x%x] Submit QI Descriptor 0x%x [0x%016lx, 0x%016lx]\n",
      VtdUnitBaseAddress,
      QueueTail,
      Qi128Desc->Low,
      Qi128Desc->High
    ...
    DEBUG ((
      DEBUG_VERBOSE,
      "[0x%x] Submit QI Descriptor 0x%x [0x%016lx, 0x%016lx, 0x%016lx, 0x%016lx]\n",
      VtdUnitBaseAddress,
      QueueTail,
      Qi256Desc->Uint64[0],
      Qi256Desc->Uint64[1],
